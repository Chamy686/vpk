...
global function AnimateSmallTabBar
...
void function AnimateSmallTabBar( TabData tabData )
{
...
void function UpdateMenuTabs()
{
	...
	foreach ( tabButtonPanel in tabButtonPanels )
	{
		...
		for ( int tabIndex = 0; tabIndex < MAX_TABS; tabIndex++ )
		{
			...
			Hud_SetWidth( tabButton, tabDef.width * multiplicationRatio )
			...
			if( Tab_IsRootLevel( tabData ) )
			{
				...
				if( childTabData != null )
				{
					...
					{
						if ( subTabIndex >= numSubTabs || !childTabData.tabDefs[subTabIndex].visible )
						{
