...
const string CM_HIDDEN_PLAYLIST_VAR = "cm_hidden_playlist"
...
void function CustomMatch_RefreshPlaylists()
{
	...
	for ( int catIdx = 0; catIdx < totalCategoryCount; catIdx++ )
	{
		...
		string logoLookup = GetCurrentPlaylistVarString( format( CM_CATEGORY_LOGO, catIdx ), "BATTLE_ROYALE" )
		category.displayLogo = GetGamemodeLogoFromImageMap( logoLookup )
	...
	Assert( file.categories.len() > 0, "Unable to retrieve custom match categories." )
...
{
	CustomMatchMap map
	map.displayName  = GetPlaylistVarString( rootPlaylist, "map_name", "Custom Match Mode" )
