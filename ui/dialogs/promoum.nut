...
struct
{
	...
	var                 activeButton
	var  				giftingInfo
	GRXScriptInboxMessage activeInfo
...
void function UpdateInboxButtons()
{
	...
	if ( inbox.gifts.len() > 0 )
	{
		...
		inbox.activeInfo.timestamp = inbox.inboxItemButtons[inbox.activeButton].timestamp
		inbox.activeInfo.itemCount = inbox.inboxItemButtons[inbox.activeButton].itemCount
		inbox.activeInfo.itemIndex = inbox.inboxItemButtons[inbox.activeButton].itemIndex
		inbox.activeInfo.gifterName = inbox.inboxItemButtons[inbox.activeButton].gifterName
		inbox.activeInfo.senderNucleusPid = inbox.inboxItemButtons[inbox.activeButton].senderNucleusPid
		inbox.activeInfo.isNew = inbox.inboxItemButtons[inbox.activeButton].isNew
		var panel = Hud_GetChild( inbox.inboxNestedPanel,  "GiftDisplayPanel" )

		RuiSetString( Hud_GetRui( panel ), "desc1", Localize( "#INBOX_PANEL_DESC", inbox.activeInfo.gifterName ).toupper() )
...
void function ListButton_OnClick( var button )
{
	...
	inbox.activeInfo.timestamp = inbox.inboxItemButtons[button].timestamp
	inbox.activeInfo.itemCount = inbox.inboxItemButtons[button].itemCount
	inbox.activeInfo.itemIndex = inbox.inboxItemButtons[button].itemIndex
	inbox.activeInfo.gifterName = inbox.inboxItemButtons[button].gifterName
	inbox.activeInfo.senderNucleusPid = inbox.inboxItemButtons[button].senderNucleusPid

	inbox.activeInfo.isNew = false
...
{
	GRXScriptInboxMessage info = inbox.activeInfo

	array<ItemFlavor> items

	foreach( int index in inbox.activeInfo.itemIndex )
	{
	...
	GRX_MarkGiftItemsAsSeen( info.timestamp, info.itemIndex )
	RemoveGiftCacheMessage( info.timestamp, info.senderNucleusPid )
	CloseActiveMenu()

	ShowRewardCeremonyDialog(
		"",
		Localize( "#INBOX_REDEMPTION_HEADER" ),
		"",
		RewardInput,
		true,
		false,
