...
struct
{
	...
	float      commBlockWarningFadeTime = 0
	bool       commBlockWarningThread = false
...
void function HideClubsWarning_Thread( var commBlockWarning )
{
	                                                                     
	if ( file.commBlockWarningId == 3 || file.commBlockWarningThread )
		return

	                       
	file.commBlockWarningThread = true

	                                            
	while ( UITime() < file.commBlockWarningFadeTime )
	{
		float waitSecs = file.commBlockWarningFadeTime - UITime()
		wait waitSecs
	}

	                                           
	if ( file.commBlockWarningId != 3 )
		Hud_SetHeight( commBlockWarning , 0 )
	file.commBlockWarningThread = false
}
...
void function UpdateClubChat()
{
	...
	if ( file.commBlockWarningId == 3 || UITime() < file.commBlockWarningFadeTime )
	{
		...
		if ( file.commBlockWarningId != 3 )
			thread HideClubsWarning_Thread( commBlockWarning )
	} else
		Hud_SetHeight( commBlockWarning, 0 )
...
void function MarkLandingTabNewness( string bodyName, bool isNew )
{
	...
	SetPanelTabNew( GetPanel( bodyName ), canFlagAsNew )
}
...
void function ClubLobby_SendChat()
{
	...
	if ( chatMessage.len() > 0 )
	{
		...
		{
			sanitizedString = replace( sanitizedString, illegalChar, "" )
		}
...
void function QueryClubTabsForNewness()
{
	...
	if ( prevStateID != file.commBlockWarningId )
	{
		...
		float warningSustain = GetConVarFloat("communicationBlock_warning_sustain")
		float warningFade = GetConVarFloat("communicationBlock_warning_fade")
		RuiSetFloat( commBlockRui, "warningSustain", warningSustain )
		RuiSetFloat( commBlockRui, "warningFade", warningFade )

		                                                                                         
		if ( prevStateID < 0 || file.commBlockWarningId == 0 )
			file.commBlockWarningFadeTime = 0
		else
			file.commBlockWarningFadeTime = UITime() + warningSustain + warningFade
