...
void function InitLoreReaderMenu( var newMenuArg )
{
	...
	AddMenuFooterOption( menu, LEFT, BUTTON_X, true, "", "#X_BUTTON_PREVIOUS", null, HasPrevious )
}
...
void function LoreReaderMenu_OpenTo( asset dtAsset, int chapter )
{
	...
	for ( int rowIdx = 0; rowIdx < numRows; rowIdx++ )
	{
		...
		switch( cmd )
		{
			...
			case "backgroundfull":
				...
				page.backgroundIsFull = true
				pageHasData = true
				break

			case "background":
				page.backgroundRef = argStr.tolower()
				page.backgroundIsFull = false
				pageHasData = true
				break
			case "musictrack":
...
void function UpdateBackgroundLoadscreen( string backgroundRef, bool backgroundIsFull = false )
{
	var rui = Hud_GetRui( Hud_GetChild( s_menu, "LoreReaderScreen" ) )

	vector shiftLayerColor = SrgbToLinear( <0, 59, 93> / 255.0 )
	float shiftLayerColorA = (backgroundIsFull)? 0.0: 0.90
	vector bleedLayerColor = SrgbToLinear( <255, 255, 255> / 255.0 )
	float bleedLayerColorA = (backgroundIsFull)? 0.0: 0.03
	vector scanLayerColor = SrgbToLinear( <255, 255, 255> / 255.0 )
	float scanLayerColorA = (backgroundIsFull)? 0.0: 0.93
	...
	RuiSetFloat( rui, "dotsAlpha", dotsAlpha )

	if ( backgroundRef.len() > 0 )
	{
		RunClientScript( "UpdateLoadscreenPreviewMaterial", Hud_GetChild( s_menu, "LoreReaderScreen" ), null, ConvertItemFlavorGUIDStringToGUID( backgroundRef ) )
	}
	else
	{
...
LoreCharacterInfo function GetLoreCharacterInfoForName( string nameRaw )
{
	...
	if ( IsValidItemFlavorCharacterRef( itemRef ) )
	{
		ItemFlavor character = GetItemFlavorByCharacterRef( itemRef )
	...
	else if ( name == "newscaster" )
	{
		info.portraitImage = $"rui/menu/quest/lore_page/lore_page_portrait_waveform"
		info.nameText = "#QUEST14_STORY_NPC_NEWSCASTER"
		info.isNPC = true
	}
	else if ( name == "jim" )
	{
		info.portraitImage = $"rui/menu/quest/lore_page/lore_page_portrait_waveform"
		info.nameText = "#QUEST14_STORY_NPC_JIM"
		info.isNPC = true
	}
	else if ( name == "mirage_hologram" )
	{
		info.nameText = "#QUEST14_STORY_NPC_MIRAGE_HOLOGRAM"
		info.isNPC = true
	}
	...
	else if ( name == "protestor" )
	{
		info.portraitImage = $"rui/menu/quest/lore_page/lore_page_portrait_waveform"
		info.nameText = "#QUEST15_STORY_NPC_PROTESTOR"
		info.isNPC = true
	}
	else if ( name == "oluwademilade" )
	{
		info.nameText = "#QUEST15_STORY_NPC_OLUWADEMILADE"
		info.isNPC = true
	}
	else if ( name == "egobunma" )
	{
		info.nameText = "#QUEST15_STORY_NPC_EGOBUNMA"
		info.isNPC = true
	}
	else
	{
...
void function AddQuestTextPinEvent( int pageIndex, int panelIndex, int previousPageIndex, int previousPanelIndex )
{
	...
	string currentID  = pageIndex == -1 ? "none" : format( "s%d_chapter%d_pg%d_panel%d", seasonNum, loreChapter, pageIndex, panelIndex )
	string previousID = previousPageIndex == -1 ? "none" : format( "s%d_chapter%d_pg%d_panel%d", seasonNum, loreChapter, previousPageIndex, previousPanelIndex )
