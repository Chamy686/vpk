...
void function CycleBadgePreviewImageThread( var button )
{
	...
	if ( string(ItemFlavor_GetAsset( badgeFlav )) == ACCOUNT_BADGE_ASSET_PATH_STRING )
		return
...
void function FilterCategorizeAndSortBadges( ItemFlavor character, array<ItemFlavor> badgeList )
{
	...
	badgeList.sort( int function( ItemFlavor a, ItemFlavor b ) {
		...
		{
			if ( string(ItemFlavor_GetAsset( a ))  > string(ItemFlavor_GetAsset( b )) )
				return 1
			if ( string(ItemFlavor_GetAsset( b ))  > string(ItemFlavor_GetAsset( a )) )
				return -1
		}
		return 0
	} )
